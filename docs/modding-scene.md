...
| **`.FNT` / palette `.TGA`** | bitmap fonts and the master 256-colour palette | **unspecified on our side.** DMJC documents the font layout and reports the real master palette lives in `palette.tga` / `softpal.tga`, not the `.ccb` files |
| **`.CCB`** | Surrender's colour-cube blocks — five in `resource.hog`, each 266,016 B decompressed | **layout decoded 2026-09-22**: five 256-entry RGB palettes at `0`–`3839`, ~32 B of scalar fields at `3840`–`3871`, a 64³ RGB→index cube at `3872`–`266015`. From `SR_CCB_load` `0x004CB9D0` / `SR_CCB_save` `0x004CBBD0` — see [`engine-map.md`](engine-map.md) §3 |
